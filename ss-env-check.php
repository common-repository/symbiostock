...
class ssEnvCheck {
	function __construct() {
		...
		// Check for pro
		if (!@$this->sspro && !get_option('ss_ignoresspro') && !isset($_GET['ss_ignoresspro'])) {
	...
	function error_notice_stored() {
		...
		?>
		<div class="notice notice-error" style="padding: 11px 15px;">
...
<?php
	...
	function error_notice_phpver() {
		?>
		<div class="notice notice-warning" style="padding: 11px 15px;">
			<?php print __('Symbiostock has been verified with PHP 5.3 to 7.3. You are currently running version '.phpversion().'. We suggest changing to a verified version to avoid bugs. Click <a href="'.admin_url( 'edit.php?post_type=product&ss_ignorephpver=1').'">here</a> to ignore this warning.','ss'); ?>
		</div>
		<?php
	}
