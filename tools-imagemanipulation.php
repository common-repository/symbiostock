...
function ssl_make_watermark($fileurl) {
...
function ssl_attach_images($postid,$title=''){
	if (!($fileurl[] = ssl_check_image_product($postid))) return;
	...
	foreach ($fileurl as $orig) {
		if ($attachmentid = ssl_add_featured_image($orig, $postid,$title)) set_post_thumbnail($postid, $attachmentid);
		@$i++;
	}
	return $i;
}

function ssl_add_featured_image($fileurl, $postid,$title) {
	$tmpfilename = $postid."_".microtime(true).".jpg";
	if ($image = ssl_make_watermark($fileurl)) {
...
function ssl_check_image_product($postid) {
